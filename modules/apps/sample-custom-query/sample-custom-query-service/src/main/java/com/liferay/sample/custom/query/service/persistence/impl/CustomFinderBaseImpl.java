/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package com.liferay.sample.custom.query.service.persistence.impl;

import com.liferay.portal.kernel.bean.BeanReference;
import com.liferay.portal.kernel.service.persistence.impl.BasePersistenceImpl;

import com.liferay.sample.custom.query.model.Custom;
import com.liferay.sample.custom.query.service.persistence.CustomPersistence;

/**
 * @author Brian Wing Shun Chan
 * @generated
 */
public class CustomFinderBaseImpl extends BasePersistenceImpl<Custom> {
	public CustomFinderBaseImpl() {
		setModelClass(Custom.class);
	}

	/**
	 * Returns the custom persistence.
	 *
	 * @return the custom persistence
	 */
	public CustomPersistence getCustomPersistence() {
		return customPersistence;
	}

	/**
	 * Sets the custom persistence.
	 *
	 * @param customPersistence the custom persistence
	 */
	public void setCustomPersistence(CustomPersistence customPersistence) {
		this.customPersistence = customPersistence;
	}

	@BeanReference(type = CustomPersistence.class)
	protected CustomPersistence customPersistence;
}